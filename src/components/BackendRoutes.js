...
// runs astar on the manifest txt file stored in localstorage, 
// I want it so that you don't even need to send the manifest txt to backend, app.py should be able to handle that itself but i figure that out later
const runAstar = async (cachedState, setCachedState) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      setCachedState({
        ...cachedState,
        currStep: result.currStep, // the current step you are on (will always start at 0 if you are running a star)
  ...
  try {
    ...
    if (response.ok) {
      ...
      setCachedState({
        ...
        // manifest, and buffer will be set only if result.ship, and result.buffer are defined.
        // this is for the event runMove is called but you have already finished the operation
        // make instruction is still called because it still returns a string when the moves list is exhausted
        instruction: makeInstruction(result.currStep, cachedState.moves),
        ...(result.ship && { manifest: result.ship }),
